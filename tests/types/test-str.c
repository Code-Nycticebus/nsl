...
static void test_str_compare(void) {
    ...
    assert(nc_str_eq(s, NC_STR("Hello, World")) == true);
    assert(nc_str_eq_ignorecase(s, NC_STR("hElLo, WoRlD")) == true);
    assert(nc_str_startswith(s, NC_STR("Hello, ")) == true);
    assert(nc_str_endswith(s, NC_STR(", World")) == true);

    assert(nc_str_contains(s, NC_STR("Hell")) == true);
    assert(nc_str_contains(s, NC_STR("TEST")) == false);

    assert(nc_str_contains(s, NC_STR("H")) == true);
    assert(nc_str_contains(s, NC_STR("T")) == false);
    assert(nc_str_contains(s, NC_STR("")) == true);

    assert(nc_str_includes(s, 'H') == true);
    assert(nc_str_includes(s, 'e') == true);
    assert(nc_str_includes(s, 'T') == false);

    assert(nc_str_is_empty(s) == false && "string should not be empty");
    assert(nc_str_is_empty(NC_STR("")) == true && "string should be empty");
}
...
static void test_str_transform(void) {
    ...
    assert(nc_str_eq(lower, NC_STR("hello, world")));
    assert(nc_str_eq(upper, NC_STR("HELLO, WORLD")));
...
static void test_str_copy(void) {
    ...
    assert(nc_str_eq(full, NC_STR("Hello, World")));
    assert(nc_str_getc(full, 0) == 'H');
    assert(nc_str_getc(full, 1) == 'e');
    assert(nc_str_getc(full, 7) == 'W');
    assert(nc_str_getc(full, full.len) == '\0');
...
static void test_str_append(void) {
    ...
    assert(nc_str_eq(append, NC_STR("filename.c")) && "did not append correctly");

    nc_Str prepend = nc_str_prepend(NC_STR("filename.c"), NC_STR("used_mark__"), &arena);
    assert(nc_str_eq(prepend, NC_STR("used_mark__filename.c")) && "did not prepend correctly");

    nc_Str wrap = nc_str_wrap(NC_STR("command here"), NC_STR("'"), &arena);
    assert(nc_str_eq(wrap, NC_STR("'command here'")) && "did not wrap correctly");
...
static void test_str_chop_right(void) {
    ...
    assert(nc_str_eq(t, NC_STR("text")));
    assert(nc_str_eq(rest, NC_STR("This is")));
    assert(nc_str_eq(rest2, NC_STR("Hello")));
    assert(nc_str_eq(rest3, NC_STR("")));
}
...
static void test_str_number_converting(void) {
    ...
    assert(nc_str_eq(number, NC_STR("64")));

    nc_Str n = nc_str_append(number, NC_STR(" bytes"), &arena);
    assert(nc_str_eq(n, NC_STR("64 bytes")));

    assert(nc_str_u64(n) == 64);
    assert(nc_str_chop_u64(&n) == 64);
    assert(n.len == 6);
    assert(nc_str_eq(n, NC_STR(" bytes")));

    nc_Str f = NC_STR("420.69");
    assert(nc_str_f64(f) == 420.69);

    assert(nc_str_chop_f64(&f) == 420.69);
    assert(f.len == 0);
    assert(nc_str_eq(f, NC_STR("")));
...
static void test_str_find(void) {
    nc_Str s = NC_STR("Hello, World");
    assert(nc_str_find(s, NC_STR("Hello")) == 0);
    assert(nc_str_find_last(s, NC_STR("Hello")) == 0);
    assert(nc_str_find(s, NC_STR("World")) == 7);
    assert(nc_str_find_last(s, NC_STR("World")) == 7);
    assert(nc_str_find(s, NC_STR("TEST")) == NC_STR_NOT_FOUND);
}
...
static void test_str_substring(void) {
    ...
    assert(nc_str_eq(substring, NC_STR("Hell")));
    assert(nc_str_eq(invalid1, NC_STR("")));
    assert(nc_str_eq(invalid2, NC_STR("")));
}
...
static void test_str_justify(void) {
    ...
    assert(nc_str_eq(center, NC_STR("  Hello   ")));
    nc_Str left = nc_str_ljust(NC_STR("Hello"), width, ' ', &arena);
    assert(nc_str_eq(left, NC_STR("Hello     ")));
    nc_Str right = nc_str_rjust(NC_STR("Hello"), width, ' ', &arena);
    assert(nc_str_eq(right, NC_STR("     Hello")));
    nc_arena_free(&arena);
}
...
static void test_str_repeat(void) {
    ...
    assert(nc_str_eq(tf_fleet, NC_STR("|-#-| |-#-| |-#-| |-#-| ")));
    nc_arena_free(&arena);
}
...
static void test_str_reverse(void) {
    ...
    assert(nc_str_eq(s, NC_STR("dlroW ,olleH")));
    nc_arena_free(&arena);
}
...
static void test_str_format(void) {
    ...
    assert(nc_str_eq(s, NC_STR("420 69")) && "Should format correctly!");
    nc_arena_free(&arena);
}

static void test_str_take(void) {
    nc_Str s = NC_STR("what a wonderful string");
    assert(nc_str_eq(nc_str_take(&s, 2), NC_STR("wh")));
    assert(nc_str_eq(nc_str_take_right(&s, 2), NC_STR("ng")));

    assert(nc_str_eq(nc_str_take(&s, 8), NC_STR("at a won")));
    assert(nc_str_eq(nc_str_take_right(&s, 5), NC_STR(" stri")));

    assert(nc_str_eq(s, NC_STR("derful")));
}
...
static void test_str_try_take(void) {
    ...
    nc_Str chunk = {0};
    assert(nc_str_try_take(&s, 7, &chunk) == true);
    assert(nc_str_eq(chunk, NC_STR("what a ")));

    assert(nc_str_try_take_right(&s, 7, &chunk) == true);
    assert(nc_str_eq(chunk, NC_STR(" string")));

    assert(nc_str_eq(s, NC_STR("wonderful")));
    assert(nc_str_try_take_right(&s, 9, &chunk) == true);

    assert(nc_str_try_take_right(&s, 2, &chunk) == false);
}
