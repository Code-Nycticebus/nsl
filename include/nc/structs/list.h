...
#define nc_list_from_array(list, _arena, count, array)                         \
    do {                                                                       \
        (list)->len = count;                                                   \
        (list)->cap = 0;                                                       \
        (list)->arena = _arena;                                                \
        (list)->items = NULL;                                                  \
        nc_list_resize(list, count);                                           \
        for (usize __e_i = 0; __e_i < (count); __e_i++) {                      \
            (list)->items[__e_i] = (array)[__e_i];                             \
        }                                                                      \
    } while (0)

#define nc_list_from_static(list, arena, ...)                                  \
    do {                                                                       \
        nc_list_from_array(list, arena, NC_ARRAY_LEN(__VA_ARGS__),             \
                           (__VA_ARGS__));                                     \
    } while (0)
...
#define nc_list_resize(list, size)                                             \
    do {                                                                       \
        ...
        (list)->cap = size;                                                    \
        (list)->items =                                                        \
            nc_arena_realloc_chunk((list)->arena, (list)->items,               \
...
#define nc_list_extend_da(list, other)                                         \
    do {                                                                       \
